...
//  Inbound
import { DeleteImagePort } from '@/src/domain/ports/inbound/image/delete-image.port';
...
export class DeleteImageUseCase extends DeleteImagePort {
  ...
  async execute(
    ...
    //  Update usedBytes
    account.usedBytes = (
      Number(account.usedBytes) - Number(image.filesize)
    ).toString();
    ...
    return { message: this.i18n.t('image.deletedSuccessfully') };
  }
}
